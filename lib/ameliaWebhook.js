...
import { processAmeliaWebhookEvent } from "./ameliaImport.js";
import { filterAmeliaRequestHeaders } from "./ameliaNormalize.js";
...
export async function logAmeliaWebhookEvent({ payload, headers = {} }) {
  ...
  const request_headers = filterAmeliaRequestHeaders(headers);
  ...
  const { data, error } = await supabase
    ...
    .select("id, event_type, event_action")
    .single();
...
export async function handleAmeliaWebhookRequest({ method, query = {}, body, headers = {} }) {
  ...
  try {
    const logged = await logAmeliaWebhookEvent({ payload, headers });
    const importResult = await processAmeliaWebhookEvent({
      eventId: logged.id,
      payload,
      headers,
      eventType: logged.event_type,
      eventAction: logged.event_action
    });
    ...
    return {
      status: 500,
      body: { ok: false, error: err.message || "Failed to process webhook event" }
    };
  }
}
