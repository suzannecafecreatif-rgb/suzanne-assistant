...
export default function Rentabilite({ ateliers, catalogue = [] }) {
  const rows = useMemo(() => {
    const enriched = getFinancialSessions(enrichAllSessions(ateliers, catalogue));
    return enriched
      .map((a) => ({
        ...a,
  ...
  return (
    <div>
      ...
      <p className="page-sub">Classement de toutes tes activités par revenu horaire réel.</p>

      {rows.length === 0 ? (
        <div className="empty-card">
          <p className="empty-title">Pas encore de classement</p>
          <p className="empty-body">Renseigne le temps passé sur tes activités pour voir apparaître leur rentabilité ici.</p>
        </div>
      ) : (
        <>
          {best && rows.length > 1 && best.revenuHoraire !== worst.revenuHoraire && (
            <div className="mini-list" style={{ marginBottom: "1rem" }}>
              <div className="mini-list-item">
                <div>
                  <p className="mini-list-theme">Continue à programmer {best.activityName}</p>
                  <p className="mini-list-reason">{Math.round(best.revenuHoraire)} €/h, ton meilleur revenu horaire</p>
                </div>
              </div>
              <div className="mini-list-item">
                <div>
                  <p className="mini-list-theme">Repense {worst.activityName}</p>
